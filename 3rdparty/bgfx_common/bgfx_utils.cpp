...
#include "Material.h"//mjt
#include "Shader.h"//mjt
#include "QCamera.h" //mjt
#include "InlineFunctions.h" //mjt
const float MULTIPLIER = 1.0f; // mjt  mesh radius' multiplier for frustum culling 
...
static bgfx::ShaderHandle loadShader(bx::FileReaderI* _reader, const char* _name)
{
	...
	bx::strCopy(filePath, BX_COUNTOF(filePath), ASSETSDIR.c_str());//mjt
	bx::strCat(filePath, BX_COUNTOF(filePath), shaderPath);//mjt
	//bx::strCopy(filePath, BX_COUNTOF(filePath), shaderPath);
	bx::strCat(filePath, BX_COUNTOF(filePath), _name);
	bx::strCat(filePath, BX_COUNTOF(filePath), ".sc.bin"); // mjt
		...
		&& err.isOk())
	{
		...
		case BGFX_CHUNK_MAGIC_PRI:
		{
			...
			for (uint32_t ii = 0; ii < num; ++ii)
			{
				...
				Primitive prim;
				prim.name = name;//mjt
				prim.materialName = material;//mjt
...
//mjt
void Mesh::submitWithMaterials(QCamera* camera, Environment* env, std::vector<MeshMaterial*> materials, bgfx::ViewId _id, const float* _mtx, uint64_t _state) const
{
	if (BGFX_STATE_MASK == _state)
	{
		_state = 0
	...
	for (GroupArray::const_iterator it = m_groups.begin(), itEnd = m_groups.end(); it != itEnd; ++it)
	{
		...
		float x = it->m_sphere.center.x + _mtx[12];
		float y = it->m_sphere.center.y + _mtx[13];
		float z = it->m_sphere.center.z + _mtx[14];

		if (materials[cc]->frustumCulling && camera->frustum.SphereInFrustum(x, y, z, it->m_sphere.radius * MULTIPLIER) == false)
		{
			visible[cc] = false;
			continue;
		}
		...
		const Group& group = *it;
		const MeshMaterial* mat = materials[cc];
		bgfx::ProgramHandle _program = mat->shader->program;
		mat->shader->SetUniforms(materials[cc], env);

		for (int q = 0; q < group.m_prims.size(); q++)
		{
			bgfx::setTransform(_mtx);
			bgfx::setState(_state);
			...
			{
				if (isValid(mat->shader->samplers[s]))
					bgfx::setTexture(s, mat->shader->samplers[s], mat->textures[s]->texture);
			}
			const Primitive& p = group.m_prims[q];
			bgfx::setIndexBuffer(group.m_ibh, p.m_startIndex, p.m_numIndices);
			bgfx::setVertexBuffer(0, group.m_vbh);
			bgfx::submit(_id, _program, 0, BGFX_DISCARD_ALL);
		}
	}
}

void Mesh::submitWithMaterialsInstanced(Environment* env, std::vector<MeshMaterial*> materials, bgfx::ViewId _id,
	uint64_t _state, uint8_t* instanceData, int numInstances, int instanceStride) const
{
	int cc = -1;
	bgfx::InstanceDataBuffer idb;
	if (bgfx::getAvailInstanceDataBuffer(numInstances, instanceStride) == numInstances)
	{
		bgfx::allocInstanceDataBuffer(&idb, numInstances, instanceStride);
		uint8_t* data = idb.data;
		memcpy(data, instanceData, numInstances * instanceStride);
		...
		for (GroupArray::const_iterator it = m_groups.begin(), itEnd = m_groups.end(); it != itEnd; ++it)
		{
			...
			const MeshMaterial* mat = materials[cc];
			bgfx::ProgramHandle _program = mat->shader->program;
			mat->shader->SetUniforms(mat, env);
			...
			for (int q = 0; q < group.m_prims.size(); q++)
			{
				...
				bgfx::submit(_id, _program, BGFX_DISCARD_ALL);
			}
		}
	}
	else
		Log("  ERROR: getAvailInstanceDataBuffer() != numInstances");
}
...
Mesh* meshLoad(bx::ReaderSeekerI* _reader, bool _ramcopy)
{
	...
	mesh->visible = new bool[mesh->m_groups.size()]; // mjt
	return mesh;
}
