...
#include <tinystl/string.h>//mjt
namespace stl = tinystl;

#include <string> //mjt
#include <vector> //mjt
class MeshMaterial; //mjt
class QCamera; //mjt
class Environment; //mjt
...
struct Mesh
{
	...
	void submitWithMaterials(QCamera* camera, Environment* env, std::vector<MeshMaterial*> materials, bgfx::ViewId _id, const float* _mtx, uint64_t _state) const; //mjt
	void submitWithMaterialsInstanced(Environment *env, std::vector<MeshMaterial*> materials, bgfx::ViewId _id, uint64_t _state, uint8_t*instanceData, int numInstances, int instanceStride) const; //mjt
	bool *visible;//mjt
};
